...
with lp.figure(f"ctftilt_lamella1",tight_layout=False):
    ...
    ax_tilt.scatter(tem_info[index,2],ctffind5_data[index,1],marker='o')
    outliers_index=np.delete(fullindex,new_index)
    ...
    outliers_index=np.delete(fullindex,index)
    ax_error.scatter(tem_info[index,2],angle_diff[index],marker='o')
    print(ctffind5_data[index[0],1]<-10.0)
    print(index[0])
    axis_error_index=index[0][ctffind5_data[index[0],1]<-10.0]
    ax_error.scatter(tem_info[axis_error_index,2],axis_diff[axis_error_index],marker='x',color='k')
